...
def get_impression_stats(yt_anal, video_id: str, days_back: int = 30) -> dict:
    """
    Récupère les impressions et CTR depuis YouTube Analytics.
    """
    end_date   = datetime.now().strftime("%Y-%m-%d")
    start_date = (datetime.now() - timedelta(days=days_back)).strftime("%Y-%m-%d")
    ...
    try:
        ...
        if rows:
            r = rows[0]
            return {
                """videoThumbnailImpressions""": int(r[1]),
                "ctr":         round(float(r[2]) * 100, 2),  # en %
            }
    except Exception as e:
        warn(f"Impressions erreur ({video_id}) : {e}")
...
def snapshot(yt, yt_anal, video_ids: list = None):
    ...
    for i, vid_id in enumerate(videos_to_track, 1):
        ...
        entry = {
            ...
            """videoThumbnailImpressions""": impr.get(videoThumbnailImpressions, 0),
            "ctr":         impr.get("ctr", 0),
            "sources":     srcs,
        }
...
def generate_report(stats: dict, yt):
    ...
    for vid_id, data in stats.items():
        ...
        impr_b  = s0.get(videoThumbnailImpressions, 0)
        impr_a  = s1.get(videoThumbnailImpressions) if has_before_after else None
